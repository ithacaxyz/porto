...
export function NotFound() {
  ...
  if (isSignTypedData) {
    ...
    if (isX402Payment) {
      console.log('This is an x402 Payment (TransferWithAuthorization):', request.params?.[1])
    }
    ...
    <Layout loading={respond.isPending} loadingTitle="Responding...">
      <Layout.Header>
        <Layout.Header.Default
          content={isX402Payment ? (
                "You're about to authorize a payment from your Porto account, please review the details below carefully."
              ) : isSignTypedData ? (
                "Special handling for eth_signTypedData_v4! 
                  Blaine detected this signature request."
              ) : (
                  Blaine olivia here! UI support for method "{request?.method}" is not implemented yet.
                  You may still proceed by rejecting or responding.
                
              )}
          icon={isX402Payment ? X402Logo : LucideTriangleAlert}
          title={isX402Payment ? "Paywalled Content Detected" : isSignTypedData ? "Sign Typed Data" : "Method Not implemented"}
          variant="warning"
        />
      </Layout.Header>
               ...
               {isX402Payment ? (
                ...
                // For now, show some basic info even if we can't parse everything
                const showBasicInfo = true
                
                // Format amount with proper decimals if possible
                const formatAmount = (value, decimals = 18) => {
                  ...
                  try {
                    ...
                    return `${whole}.${remainder.toString().padStart(decimals, '0').slice(0, 6)} tokens`
                  } catch {
                    return value
                  }
                }
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex h-5.5 items-center justify-between text-[14px]">
                        <span className="text-[14px] text-th_base-secondary leading-4">Amount</span>
                      ...
                      {/* Type */}
                      <div className="flex h-5.5 items-center justify-between text-[14px]">
                        <span className="text-[14px] text-th_base-secondary leading-4">Type</span>
                        <span className="font-medium leading-4">Payment (x402)</span>
                      </div>
                      
                      {/* From Address */}
                      {finalMessage?.from && (
                        <div className="flex h-5.5 items-center justify-between text-[14px]">
                          <span className="text-[14px] text-th_base-secondary leading-4">From</span>
                          <span className="font-medium font-mono leading-4">{formatAddress(finalMessage.from)}</span>
                        </div>
                      )}
                      ...
                      {finalMessage?.to && (
                        <div className="flex h-5.5 items-center justify-between text-[14px]">
                          <span className="text-[14px] text-th_base-secondary leading-4">To</span>
                          <span className="font-medium font-mono leading-4">{formatAddress(finalMessage.to)}</span>
                        </div>
                      )}
                      ...
                      {finalDomain?.name && (
                        <div className="flex h-5.5 items-center justify-between text-[14px]">
                          <span className="text-[14px] text-th_base-secondary leading-4">Token</span>
                          <span className="font-medium leading-4">{finalDomain.name}</span>
                        </div>
                      )}
                      ...
                      {finalDomain?.verifyingContract && (
                        <div className="flex h-5.5 items-center justify-between text-[14px]">
                          <span className="text-[14px] text-th_base-secondary leading-4">Contract</span>
                          <span className="font-medium font-mono leading-4">{formatAddress(finalDomain.verifyingContract)}</span>
                        </div>
                      )}
                      ...
                      {finalMessage?.validBefore && (
                        <div className="flex h-5.5 items-center justify-between text-[14px]">
                          <span className="text-[14px] text-th_base-secondary leading-4">Expires in</span>
                          <span className="font-medium leading-4">
                            {(() => {
                              const expiresAt = Number.parseInt(finalMessage.validBefore) * 1000
