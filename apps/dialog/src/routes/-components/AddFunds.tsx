...
export function AddFunds(props: AddFunds.Props) {
  ...
  const [view, setView] = React.useState<'default' | 'success'>('default')
  ...
  const isLoading = loading || deposit.isPending

  return (
    <Layout loading={isLoading} loadingTitle="Adding funds...">
      {view === 'default' ? (
        <React.Fragment>
          <Layout.Header>
        ...
        <React.Fragment>
          <div className="px-4">
            <div className="flex flex-row items-center gap-2">
              <div className="flex size-8 items-center justify-center rounded-full bg-green4">
                <CheckIcon className="size-5 text-green11" />
              </div>
              <p className="my-auto font-medium text-lg">
                Received {desiredAmount} EXP
              </p>
            </div>
          </div>
          <Layout.Content className="px-4">
            <p className="">
              Faucet has blessed you with some funds.{' '}
              <span className="text-gray10">
                You can safely close this prompt
              </span>
            </p>
          </Layout.Content>
        </React.Fragment>
