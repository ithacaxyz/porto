import { TryItOut } from '../../../../components/TryItOut'

# `Dialog.iframe`

The [`iframe`](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/iframe) dialog is Porto's default rendering option that displays the wallet interface in a modal dialog using an `iframe`.

## Usage

```ts twoslash
import { Porto, Dialog, Mode } from 'porto'

const porto = Porto.create({
  mode: Mode.dialog({
    host: 'https://dialog.porto.sh', // Porto dialog host URL
    renderer: Dialog.iframe()
  })
})
```

## Behavior

- The dialog opens automatically when a request requires user confirmation
- The dialog closes when:
  - The user completes the requested action
  - The user clicks outside the dialog
  - The user presses the Escape key
- When the dialog closes due to a user cancellation, all pending requests will be rejected with a `UserRejectedRequestError`

## Example

```ts twoslash
import { Porto, Dialog, Mode } from 'porto'

const porto = Porto.create({
  mode: Mode.dialog({
    host: 'https://dialog.porto.sh',
    renderer: Dialog.iframe()
  })
})
```
