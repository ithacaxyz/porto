...
React hook for disconnecting an account. Uses [`wallet_disconnect`](/sdk/rpc/wallet_disconnect) under the hood.
...
export function App() {
  ...
  return (
    <button onClick={() => disconnect.mutate({ connector })}>Disconnect</button>
  )
}
```
