...
`{ currency: 'ETH' | 'USD' | 'USDC' | 'USDT', value: string } | null`
...
- Defaults to `porto.config.permissionsFeeLimit` (1 USD).
- Provide `null` to have the fee inclusive with `permissions.spend`.
