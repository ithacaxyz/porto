...
`{ publicKey: Address, type: 'secp256k1' } | undefined`
...
`{
  calls?: { signature?: string, to?: Address }[],
  signatureVerification?: { addresses: Address[] },
  spend?: { limit: Hex, period: 'minute' | 'hour' | 'day' | 'week' | 'month' | 'year', token?: Address }[]
}`

Permission configuration specifying which actions are allowed.

#### calls

Array of call permissions that define which contracts and functions can be called.

#### signatureVerification

Optional configuration for signature verification permissions.

#### spend

Array of spend permissions that define token spending limits.
...
```tsx
UseMutationResult<unknown, Error, GrantPermissionsParameters>
```
