...
describe('parseJsonTheme', () => {
  ...
  test('extracts hideEmailInput property', () => {
    const theme = {
      colorScheme: 'light',
      hideEmailInput: true,
    }
    const result = parseJsonTheme(JSON.stringify(theme), testMappings)
    expect(result.hideEmailInput).toBe(true)
  })

  test('extracts hideBugReportIcon property', () => {
    const theme = {
      colorScheme: 'light',
      hideBugReportIcon: true,
    }
    const result = parseJsonTheme(JSON.stringify(theme), testMappings)
    expect(result.hideBugReportIcon).toBe(true)
  })

  test('extracts hideAccountSwitcher property', () => {
    const theme = {
      colorScheme: 'light',
      hideAccountSwitcher: true,
    }
    const result = parseJsonTheme(JSON.stringify(theme), testMappings)
    expect(result.hideAccountSwitcher).toBe(true)
  })

  test('extracts hideSignInButton and hideCreateAccountButton properties', () => {
    const theme = {
      colorScheme: 'light',
    ...
    const result = parseJsonTheme(JSON.stringify(theme), testMappings)
    expect(result.hideSignInButton).toBe(true)
    expect(result.hideCreateAccountButton).toBe(false)
  })

  test('extracts labels object', () => {
    const theme = {
      colorScheme: 'light',
      labels: {
        bugReportEmail: 'bugs@example.com',
        createAccount: 'Custom create',
  ...
  test('returns undefined for boolean properties when not provided', () => {
    const theme = {
      colorScheme: 'light',
    }
    const result = parseJsonTheme(JSON.stringify(theme), testMappings)
    expect(result.hideEmailInput).toBeUndefined()
    expect(result.hideBugReportIcon).toBeUndefined()
    expect(result.hideAccountSwitcher).toBeUndefined()
    expect(result.hideSignInButton).toBeUndefined()
    expect(result.hideCreateAccountButton).toBeUndefined()
    expect(result.labels).toBeUndefined()
  })

  test('ignores non-boolean values for hide properties', () => {
    const theme = {
      colorScheme: 'light',
      hideBugReportIcon: 1, // number instead of boolean
      hideEmailInput: 'true', // string instead of boolean
    }
    const result = parseJsonTheme(JSON.stringify(theme), testMappings)
    expect(result.hideEmailInput).toBeUndefined()
    expect(result.hideBugReportIcon).toBeUndefined()
