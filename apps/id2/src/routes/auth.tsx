...
//   // const account = await WalletActions.connect(client)
//   const { address, status, chainId, isConnected } = getAccount(Wagmi.config)
//   console.info({ address, chainId, isConnected, status })
...
export const Route = createFileRoute('/auth')({
  ...
  async beforeLoad() {
    ...
    const { address, status, chainId, isConnected } = getAccount(Wagmi.config)
    console.info({ address, chainId, isConnected, status })
    if (address && status === 'connected') throw redirect({ to: '/' })
        ...
        <div className="mx-auto flex h-full w-full max-w-[328px] flex-col justify-center gap-y-6 max-lg:gap-y-20">
          <div className="flex flex-col items-center">
            ...
            <p className="max-w-[24ch] text-center text-[18px] text-base text-gray11 tracking-[-2.8%]">
              Porto is a seamless, friendly way to use digital assets on-the-go.
            </p>
          </div>
