...
>(config: ExactPartial<Porto.Config<chains>> = {}) {
  ...
  return createConnector<Provider, Properties>((wagmiConfig) => {
    ...
    const Schema_promise = (() => import('../core/internal/schema/schema.js'))()
    const RpcSchema_promise = (() => import('../core/RpcSchema.js'))()

    const porto_promise = (async () => {
    ...
    return {
      ...
      async getProvider() {
        const porto = await porto_promise
