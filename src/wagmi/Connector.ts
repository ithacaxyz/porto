    ...
    return {
      ...
      async getProvider() {
        async function initProvider(): Promise<Provider> {
          ...
          const PortoModule = await (async () => {
            const module = await import('../core/Porto.js')
            return module
          })()

          portoInstance = PortoModule.create({
            ...config,
          ...
          portoProvider = portoInstance.provider
          return portoProvider
        }

        if (!portoProvider) {
          if (!providerPromise) providerPromise = initProvider()
          portoProvider = await providerPromise
        }
