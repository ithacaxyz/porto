...
export * as Actions from './Actions.native.js'
...
import * as Mode from '../core/Mode.native.js'
import type * as PortoType from '../core/Porto.js'
...
>(parameters: PortoParameters<chains> = {}) {
  type Provider = ReturnType<typeof PortoType.create>['provider']
  type Properties = {
    ...
    getPortoInstance(): Promise<
      PortoType.Porto<readonly [Chains.Chain, ...Chains.Chain[]]>
    >
  ...
  return createConnector<Provider, Properties>((wagmiConfig) => {
    ...
    const transports = (() => {
      ...
      return restParameters.transports
    })()
    const mode =
      modeParameter ??
      Mode.reactNative({
